...
def choise_some_group(group_list):
...
def choice_some_contact(app, group, old_contacts_not_in_group, orm):
...
def check_add_contact_in_group_success(app, check_ui, group, new_contacts_in_group, new_contacts_not_in_group,
    ...
    if check_ui:
        ...
        db_contacts = list(
            map(lambda contact: Contact(id=contact.id, firstname=contact.firstname, lastname=contact.lastname,
                                        ...
                                        email_1=None, email_2=None, email_3=None), new_contacts_in_group))
...
def test_add_contact_in_some_group(app, orm, check_ui):
    #choice some group
    group_list = get_group_list(app, orm)
    group = choise_some_group(group_list)
    #get old lists
    old_contacts_in_group = orm.get_contacts_in_group(group)
    old_contacts_not_in_group = orm.get_contacts_not_in_group(group)
    #choice contact and perhaps update old_contacts_not_in_group
    contact, old_contacts_not_in_group = choice_some_contact(app, group, old_contacts_not_in_group, orm)
    ...
    check_add_contact_in_group_success(app, check_ui, group, new_contacts_in_group, new_contacts_not_in_group,
                                       old_contacts_in_group, old_contacts_not_in_group)